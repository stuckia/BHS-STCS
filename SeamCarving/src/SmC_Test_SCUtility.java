...
public class SmC_Test_SCUtility {


    // create random W-by-H array of tiles
    public static SmC_Picture randomPicture(int W, int H) {
        SmC_Picture p = new SmC_Picture(W, H);
        for (int i = 0; i < W; i++)
            for (int j = 0; j < H; j++) {
    ...
    public static SmC_Picture toEnergyPicture(SeamCarver sc) {
    ...
    public static SmC_Picture doubleToPicture(double[][] grayValues) {
        ...
        SmC_Picture picture = new SmC_Picture(width, height);
    ...
    public static SmC_Picture seamOverlay(SmC_Picture p, boolean horizontal, int[] seamIndices) {
        SmC_Picture overlaid = new SmC_Picture(p.width(), p.height());
